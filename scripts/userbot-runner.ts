...
// "authorization key" menangkap bentuk kalimat Telegram yang tanpa kode
// (mis. "The specified authorization key is not registered in the system")
// yang terbukti muncul di produksi saat key PFS kadaluarsa.
function sessionInvalid(error: unknown) { return /AUTH_KEY_UNREGISTERED|AUTH_KEY_INVALID|AUTH_KEY_DUPLICATED|SESSION_REVOKED|SESSION_EXPIRED|USER_DEACTIVATED|USER_DEACTIVATED_BAN|CONCURRENT USAGE OF THE CURRENT SESSION|authorization key/i.test(errorText(error)); }
...
export async function runBuyerSession(buyerId: string, isStillWanted: () => boolean = () => true) {
  ...
  async function refreshTargets() { const response = await fetch(`${baseUrl}/api/internal/comment-monitor?buyerId=${encodeURIComponent(buyerId)}`, { headers }); if (!response.ok) return; const payload = await response.json() as { ready?: boolean; targets?: Target[] }; if (payload.ready === false) { targets = []; return; } targets = []; for (const target of payload.targets ?? []) targets.push(await ensureTarget(target)); }
  ...
  try {
    // connect/checkAuthorization bisa GAGAL LEMPAR (mis. AUTH_KEY_UNREGISTERED),
    // bukan cuma return false. Tanpa penanganan ini sesi mati permanen tidak
    // pernah dilaporkan ke server: buyer tetap dianggap CONNECTED, tidak menerima
    // peringatan hubungkan ulang, dan host diam-diam mencoba ulang terus.
    try { await client.connect(); if (!await client.checkAuthorization()) throw new Error("AUTH_KEY_UNREGISTERED"); }
    catch (error) {
